...
fn test_localstorage_backend() -> TegResult<()> {
    ...
    console_log!("   ✓ Found {} users in localStorage", count);
    
    // Test transactions
    console_log!("   Testing transactions...");
    ...
    for row_result in results {
        let row = row_result?;
        console_log!("     Final user: {:?}", row);
        final_count += 1;
    }
    console_log!("   Total users after transaction: {}", final_count);
    
    console_log!("   ✓ localStorage backend test completed");
    Ok(())
}

fn test_indexeddb_backend() -> TegResult<()> {
    console_log!("   Creating IndexedDB-style database...");
    let mut db = Database::open("browser://my-web-app-db")?;
    
    console_log!("   Creating table for web app data...");
    ...
    console_log!("   Querying session data...");
    let results = db.query("SELECT token FROM sessions WHERE created_at > 1640995150")?;
    for row_result in results {
        let row = row_result?;
        console_log!("     Active session: {:?}", row);
    }
    
    console_log!("   ✓ IndexedDB-style backend test completed");
    Ok(())
}
...
pub fn test_tegdb_performance() -> Result<(), JsValue> {
    ...
    console_log!("Inserting 100 rows...");
    for i in 1..=100 {
    ...
    console_log!("✓ Inserted 100 rows in {:.2}ms", insert_time);
    ...
    for row_result in results {
        ...
        console_log!("Sample result: {:?}", row);
        total_count += 1;
    }
    
    let query_time = js_sys::Date::now() - query_start;
    console_log!("✓ Queried {} sample rows in {:.2}ms", total_count, query_time);
    
    console_log!("🎯 Performance test completed successfully!");
    Ok(())
}
