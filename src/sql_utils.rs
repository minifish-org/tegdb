...
pub fn escape_sql_string(s: &str) -> String {
    ...
    for ch in s.chars() {
        match ch {
            '\'' => {
                // TegDB parser prefers backslash escaping over doubled quotes
                result.push_str("\\'");
            }
            '\\' => {
                // Escape backslashes
            ...
            '\n' => {
                // Literal newlines (don't escape - let parser handle them)
                result.push('\n');
            }
            '\t' => {
            ...
            '\r' => {
                // Literal carriage returns (don't escape - let parser handle them)
                result.push('\r');
            }
            _ => {
                // All other characters (including Unicode/emojis) are included as-is
                result.push(ch);
            }
        }
    }
