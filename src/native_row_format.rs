...
/// 
/// Record format structure (similar to SQLite):
/// - Header: [record_size(varint)] [header_size(varint)] [column_types...]
/// - Data: [column_data...]
/// 
/// This format allows for:
/// - Direct column access without full deserialization
/// - Compact storage with variable-length encoding
/// - Type information embedded in the record
/// - Skip unused columns during scanning
pub struct NativeRowFormat;
...
impl NativeRowFormat {
    ...
    /// Deserialize a complete row (when all columns are needed)
    ...
    /// Check if a row matches a condition without full deserialization
    pub fn matches_condition_fast(
        data: &[u8],
        schema: &TableSchema,
    ...
    fn parse_header(data: &[u8], schema: &TableSchema) -> crate::Result<RecordHeader> {
        let mut cursor = 0;
    ...
    fn deserialize_column_at_offset(
        ...
        if end > data.len() {
            return Err(crate::Error::Other("Column data extends beyond record".to_string()));
        }
        ...
        match column_info.type_code {
            ...
            2 => Ok(SqlValue::Integer(i16::from_le_bytes([column_data[0], column_data[1]]) as i64)),
            3 => Ok(SqlValue::Integer(i32::from_le_bytes([
                column_data[0], column_data[1], column_data[2], column_data[3]
            ]) as i64)),
            4 => Ok(SqlValue::Integer(i64::from_le_bytes([
                column_data[0], column_data[1], column_data[2], column_data[3],
                column_data[4], column_data[5], column_data[6], column_data[7]
            ]))),
            ...
            _ => Err(crate::Error::Other(format!("Unknown type code: {}", column_info.type_code)))
        }
    }
    
    /// Serialize a single value and return its type code
    fn serialize_value(value: &SqlValue, buffer: &mut Vec<u8>) -> crate::Result<u8> {
        match value {
            SqlValue::Null => Ok(0),
            SqlValue::Integer(i) => {
    ...
    fn decode_varint(data: &[u8]) -> crate::Result<(usize, usize)> {
        let mut result = 0;
        let mut shift = 0;
...
mod tests {
    ...
    fn test_serialize_deserialize_round_trip() {
        ...
        let serialized = NativeRowFormat::serialize(&row_data, &schema).unwrap();
        let deserialized = NativeRowFormat::deserialize_full(&serialized, &schema).unwrap();
        
        assert_eq!(deserialized.get("id"), Some(&SqlValue::Integer(123)));
        assert_eq!(deserialized.get("name"), Some(&SqlValue::Text("Alice".to_string())));
    ...
    fn test_partial_column_deserialization() {
        ...
        let values = NativeRowFormat::deserialize_columns(&serialized, &schema, &columns).unwrap();
