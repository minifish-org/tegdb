...
pub fn debug_protocol_parsing() -> JsValue {
    ...
    for identifier in test_identifiers {
        ...
        results.push(format!("'{}' -> protocol: '{}', path: '{}'", identifier, protocol, path));
    }
...
pub fn comprehensive_wasm_test_runner() -> JsValue {
    ...
    let test_categories: Vec<(&str, fn() -> crate::Result<()>)> = vec![
        ("Database Operations", test_wasm_database_operations as fn() -> crate::Result<()>),
        ("Transaction Patterns", test_wasm_transaction_patterns as fn() -> crate::Result<()>),
        ("Schema Operations", test_wasm_schema_operations as fn() -> crate::Result<()>),
        ("Query Operations", test_wasm_query_operations as fn() -> crate::Result<()>),
        ("Error Scenarios", test_wasm_error_scenarios as fn() -> crate::Result<()>),
        ("Advanced Database Tests", test_wasm_advanced_database_tests as fn() -> crate::Result<()>),
        ("SQL Integration Tests", test_wasm_sql_integration_tests as fn() -> crate::Result<()>),
        ("Transaction Integration Tests", test_wasm_transaction_integration_tests as fn() -> crate::Result<()>),
        ("Schema Persistence Tests", test_wasm_schema_persistence_tests as fn() -> crate::Result<()>),
        ("Query Iterator Tests", test_wasm_query_iterator_tests as fn() -> crate::Result<()>),
    ];
...
fn test_wasm_basic_crud() -> crate::Result<()> {
    ...
    // Create table
    db.execute("CREATE TABLE users_crud (id INTEGER PRIMARY KEY, name TEXT NOT NULL, age INTEGER)")?;
...
fn test_wasm_query_operations() -> crate::Result<()> {
    ...
    // Setup test data
    db.execute("CREATE TABLE query_test (id INTEGER PRIMARY KEY, name TEXT, age INTEGER, city TEXT)")?;
...
fn test_wasm_transaction_integration_tests() -> crate::Result<()> {
    ...
    // Setup
    db.execute("CREATE TABLE tx_integration_test (id INTEGER PRIMARY KEY, step INTEGER, data TEXT)")?;
    ...
    assert_eq!(result.rows()[0][0], crate::SqlValue::Text("auto-commit".to_string()));
