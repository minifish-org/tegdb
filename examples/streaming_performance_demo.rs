...
fn main() -> tegdb::Result<()> {
    ...
    println!("  ✓ Found {} high temperature readings in {:?}", high_temp_count, filter_time);
    ...
    println!("\nWhat streaming would provide:");
    println!("  ✓ iterator.skip({}).take({}) - only processes needed rows", offset, page_size);
    ...
    for (i, row) in page_result.rows().iter().take(3).enumerate() {
        println!("    Row {}: ID={}, Sensor={}, Location={}", 
                 offset + i + 1,
