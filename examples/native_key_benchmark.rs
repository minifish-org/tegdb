...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Prepare statements
    let insert_stmt = db.prepare("INSERT INTO benchmark_test (id, name, value) VALUES (?1, ?2, ?3)")?;
    ...
    for i in 1..=10000 {
        db.execute_prepared(&insert_stmt, &[SqlValue::Integer(i), SqlValue::Text(format!("item_{}", i)), SqlValue::Real(i as f64 * 1.5)])?;
    }
    let insert_time = start.elapsed();
    println!("INSERT operations: {:.2?}", insert_time);
    ...
    println!("SELECT operations: {:.2?}", select_time);
    ...
    for i in 0..100 {
        ...
        let end_id = (i + 1) * 100;
        let result = db.query_prepared(&range_stmt, &[SqlValue::Integer(start_id), SqlValue::Integer(end_id)])?;
        ...
        assert_eq!(result.rows().len(), expected_count, 
            ...
            expected_count, start_id, end_id, result.rows().len());
    }
    let range_time = start.elapsed();
    println!("Range scan operations: {:.2?}", range_time);
    ...
    println!("String key size: {} bytes", string_key_size);
    println!("Native key size: {} bytes", native_key_size);
    ...
    println!("   → Binary format savings: {:.1}%", 
