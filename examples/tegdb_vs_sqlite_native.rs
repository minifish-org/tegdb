...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let row_count = 25000;
    println!("Dataset: {} rows with 5 columns (id, name, email, score, active)", row_count);
    ...
    print_comparison_row("Table Creation", 
        tegdb_results.creation_time, sqlite_results.creation_time);
    print_comparison_row("Bulk Insert", 
        tegdb_results.insert_time, sqlite_results.insert_time);
    print_comparison_row("Full Table Scan", 
        tegdb_results.full_scan_time, sqlite_results.full_scan_time);
    print_comparison_row("Selective Columns (name, score)", 
        tegdb_results.selective_scan_time, sqlite_results.selective_scan_time);
    print_comparison_row("Primary Key Lookup", 
        tegdb_results.pk_lookup_time, sqlite_results.pk_lookup_time);
    print_comparison_row("LIMIT 1000 Query", 
        tegdb_results.limited_scan_time, sqlite_results.limited_scan_time);
    print_comparison_row("COUNT(*) Query", 
        tegdb_results.count_time, sqlite_results.count_time);
    
    println!("\n=== STORAGE EFFICIENCY ===");
    println!("TegDB (Native Format): {:>10} bytes", tegdb_results.db_size);
    println!("SQLite:                {:>10} bytes", sqlite_results.db_size);
    
    let size_ratio = sqlite_results.db_size as f64 / tegdb_results.db_size as f64;
    if size_ratio > 1.0 {
        println!("TegDB storage is {:.1}x more compact than SQLite", size_ratio);
    } else {
        println!("SQLite storage is {:.1}x more compact than TegDB", 1.0 / size_ratio);
    }
    
    println!("\n=== PERFORMANCE ANALYSIS ===");
    
    // Analyze key metrics
    let selective_ratio = tegdb_results.selective_scan_time as f64 / sqlite_results.selective_scan_time as f64;
    let full_scan_ratio = tegdb_results.full_scan_time as f64 / sqlite_results.full_scan_time as f64;
    let pk_lookup_ratio = tegdb_results.pk_lookup_time as f64 / sqlite_results.pk_lookup_time as f64;
    let limit_ratio = tegdb_results.limited_scan_time as f64 / sqlite_results.limited_scan_time as f64;
    
    println!("Key Performance Gaps (TegDB vs SQLite):");
    println!("• Selective column queries: {:.1}x SQLite speed", 1.0/selective_ratio);
    println!("• Full table scans:         {:.1}x SQLite speed", 1.0/full_scan_ratio);
    println!("• Primary key lookups:      {:.1}x SQLite speed", 1.0/pk_lookup_ratio);
    println!("• LIMIT queries:            {:.1}x SQLite speed", 1.0/limit_ratio);
...
fn test_tegdb_native(row_count: usize) -> Result<BenchmarkResults, Box<dyn std::error::Error>> {
    ...
    let _result = db.query("SELECT * FROM users").unwrap().into_query_result().unwrap();
    let full_scan_time = full_start.elapsed().as_nanos();
    
    // Selective column scan
    let selective_start = Instant::now();
    let _result = db.query("SELECT name, score FROM users").unwrap().into_query_result().unwrap();
    let selective_scan_time = selective_start.elapsed().as_nanos();
    
    // Primary key lookup
    let pk_start = Instant::now();
    let _result = db.query(&format!("SELECT name, email FROM users WHERE id = {}", row_count / 2)).unwrap().into_query_result().unwrap();
    let pk_lookup_time = pk_start.elapsed().as_nanos();
    
    // Limited scan
    let limit_start = Instant::now();
    let _result = db.query("SELECT name, score FROM users LIMIT 1000").unwrap().into_query_result().unwrap();
    let limited_scan_time = limit_start.elapsed().as_nanos();
    
    // Count query
    let count_start = Instant::now();
    let _result = db.query("SELECT COUNT(*) FROM users").unwrap().into_query_result().unwrap();
...
fn test_sqlite(row_count: usize) -> Result<BenchmarkResults, Box<dyn std::error::Error>> {
    ...
    // Primary key lookup
    let pk_start = Instant::now();
    let mut stmt = conn.prepare(&format!("SELECT name, email FROM users WHERE id = {}", row_count / 2))?;
    let rows = stmt.query_map([], |_row| Ok(()))?;
    let _count: Result<Vec<_>, _> = rows.collect();
    let pk_lookup_time = pk_start.elapsed().as_nanos();
...
fn print_comparison_header() {
    println!("{:<25} | {:>12} | {:>12} | {:>12}", "Operation", "TegDB (ms)", "SQLite (ms)", "Ratio");
    println!("{}", "-".repeat(70));
}
