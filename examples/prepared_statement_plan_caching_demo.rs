...
fn main() -> Result<()> {
    ...
    // Insert test data
    for i in 1..=1000 {
        let city = if i % 3 == 0 { "New York" } else if i % 3 == 1 { "Los Angeles" } else { "Chicago" };
        db.execute(&format!(
    ...
    for (age, city) in test_cases {
        ...
        println!("   → age > {} AND city = '{}': {} rows in {:?}", 
                age, city, result.rows().len(), duration);
    }
    ...
    for i in 1001..=1010 {
        ...
        println!("   → Insert {}: {} rows affected in {:?}", i, affected, duration);
    }
    ...
    for i in 1..=5 {
        ...
        println!("   → Update {}: {} rows affected in {:?}", i, affected, duration);
    }
    ...
    let count = if let Some(SqlValue::Integer(count)) = final_result.rows().first().and_then(|row| row.first()) {
        *count
    } else {
        0
    };
