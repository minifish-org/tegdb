...
fn main() -> Result<()> {
    ...
    for i in 1..=1000 {
        let insert_sql = format!("INSERT INTO users (id, email, name) VALUES ({}, 'user{}@example.com', 'User {}')", i, i, i);
    ...
    match db.execute("INSERT INTO users (id, email, name) VALUES (1001, 'user500@example.com', 'Duplicate')") {
    ...
    match db.execute("UPDATE users SET email = 'user999@example.com' WHERE id = 1") {
        ...
        Err(e) => {
            ...
            println!("   ✅ Update unique constraint violation detected in {:?}", update_violation_time);
            println!("   ✅ Error: {}", e);
        }
    }
    ...
    let result = db.query("SELECT id, email FROM users WHERE id = 1")?.into_query_result()?;
