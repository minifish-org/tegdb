...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    db.execute("CREATE TABLE large_dataset (
    ...
    for batch in 0..(total_records / batch_size) {
        ...
        for i in 0..batch_size {
            ...
            let category = if id % 3 == 0 { "premium" } else if id % 3 == 1 { "standard" } else { "basic" };
        ...
        tx.commit()?;
        println!("  ✓ Inserted batch {} ({} records)", batch + 1, (batch + 1) * batch_size);
    }
    ...
    println!("✓ Standard items sample: {} rows returned", result1.rows().len());
    println!("✓ Premium items sample: {} rows returned", result2.rows().len());
