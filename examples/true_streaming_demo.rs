...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    while let Some(row) = streaming_result.next() {
        let row = row?;
        println!("     Row {}: id={:?}, data={:?}, value={:?}", 
                count + 1, row[0], row[1], row[2]);
        count += 1;
        if count >= 3 {
            break;
        }
    ...
    println!("   - Time to process {} rows with LIMIT: {:?}", limited_count, limited_time);
    println!("   - Memory efficiency: Only {} rows ever existed in memory", limited_count);
