...
fn main() -> Result<()> {
    ...
    let mut db = Database::open("file:///tmp/streaming_resultset_demo.teg")?;
    ...
    println!(
        "   ✓ Query created in {:?} (no rows materialized yet)",
        start.elapsed()
    );
    ...
    for (i, row) in qr.rows().iter().enumerate() {
        ...
        if i < 3 {
            println!(
                "   • Processing row {}: sensor_id={:?}, value={:?}",
                i + 1,
                row.get(1),
                row.get(3)
            );
    ...
    println!(
        "   ✓ Retrieved {} rows with LIMIT 10 in {:?}",
        rows.len(),
        limit_time
    );
