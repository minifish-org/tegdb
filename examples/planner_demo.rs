...
fn demonstrate_planning_concept() -> Result<()> {
    ...
    for (sql, strategy, description) in queries_and_strategies {
        ...
        println!("   Pipeline: SQL → Parse → Plan({}) → Execute → Result", strategy);
        println!();
    }
...
fn demonstrate_execution_comparison() -> Result<()> {
    ...
    let test_queries = vec![
        ...
        ("Sequential Scan", "SELECT * FROM users WHERE name = 'User50'"),
        ("Range Query", "SELECT * FROM users WHERE age BETWEEN 25 AND 35"),
    ...
    for (query_type, sql) in test_queries {
        ...
        println!("     → Found {} rows in {:?}", result.rows().len(), duration);
