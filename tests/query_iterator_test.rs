...
fn test_query_iterator_basic_functionality() {
    ...
    // Setup test data
    db.execute("CREATE TABLE test_table (id INTEGER PRIMARY KEY, name TEXT, value INTEGER)").unwrap();
    db.execute("INSERT INTO test_table (id, name, value) VALUES (1, 'first', 100)").unwrap();
    db.execute("INSERT INTO test_table (id, name, value) VALUES (2, 'second', 200)").unwrap();
    ...
    assert_eq!(rows[1], vec![SqlValue::Integer(2), SqlValue::Text("second".to_string()), SqlValue::Integer(200)]);
    assert_eq!(rows[2], vec![SqlValue::Integer(3), SqlValue::Text("third".to_string()), SqlValue::Integer(300)]);
...
fn test_query_iterator_streaming() {
    ...
    // Setup test data
    db.execute("CREATE TABLE streaming_test (id INTEGER PRIMARY KEY, data TEXT)").unwrap();
    for i in 1..=5 {
        db.execute(&format!("INSERT INTO streaming_test (id, data) VALUES ({}, 'data_{}')", i, i)).unwrap();
    }
    ...
    assert_eq!(collected_rows[0], vec![SqlValue::Integer(1), SqlValue::Text("data_1".to_string())]);
    assert_eq!(collected_rows[1], vec![SqlValue::Integer(2), SqlValue::Text("data_2".to_string())]);
    assert_eq!(collected_rows[2], vec![SqlValue::Integer(3), SqlValue::Text("data_3".to_string())]);
...
fn test_query_iterator_backward_compatibility() {
    ...
    // Setup test data
    db.execute("CREATE TABLE compat_test (id INTEGER PRIMARY KEY, name TEXT)").unwrap();
    db.execute("INSERT INTO compat_test (id, name) VALUES (1, 'Alice')").unwrap();
    ...
    let rows = query_result.rows();
    assert_eq!(rows[0], vec![SqlValue::Integer(1), SqlValue::Text("Alice".to_string())]);
    assert_eq!(rows[1], vec![SqlValue::Integer(2), SqlValue::Text("Bob".to_string())]);
...
fn test_query_iterator_with_where_clause() {
    ...
    // Setup test data
    db.execute("CREATE TABLE where_test (id INTEGER PRIMARY KEY, value INTEGER)").unwrap();
    for i in 1..=10 {
        db.execute(&format!("INSERT INTO where_test (id, value) VALUES ({}, {})", i, i * 10)).unwrap();
    }
...
fn test_transaction_query_iterator() {
    ...
    // Setup test data
    db.execute("CREATE TABLE tx_test (id INTEGER PRIMARY KEY, name TEXT)").unwrap();
    db.execute("INSERT INTO tx_test (id, name) VALUES (1, 'initial')").unwrap();
    ...
    assert_eq!(rows[1], vec![SqlValue::Integer(2), SqlValue::Text("in_transaction".to_string())]);
