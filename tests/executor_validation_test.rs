...
fn setup_test_db() -> Result<(Database, TempDir)> {
    ...
    // Create test table
    db.execute("CREATE TABLE users (id INTEGER PRIMARY KEY, name TEXT NOT NULL, email TEXT UNIQUE)")?;
...
fn test_insert_validation() -> Result<()> {
    ...
    let result = db.execute("INSERT INTO users (id, name, email) VALUES (3, 'Charlie', 'alice@example.com')");
    assert!(result.is_err(), "Should fail for UNIQUE violation");
    println!("UNIQUE validation: {:?}", result);
    
    // Test unknown column
    let result = db.execute("INSERT INTO users (id, name, unknown_column) VALUES (4, 'David', 'something')");
...
fn test_select_memory_optimization() -> Result<()> {
    ...
    // Insert test data
    for i in 1..=100 {
        db.execute(&format!("INSERT INTO users (id, name, email) VALUES ({}, 'User{}', 'user{}@example.com')", i, i, i))?;
    }
