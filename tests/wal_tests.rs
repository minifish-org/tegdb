...
fn test_wal_deletions() {
    ...
    assert!(
        key_map.get(&key).is_none(),
        "Key should not exist in map after deletion"
    );
...
fn test_wal_multiple_files() {
    ...
    // Verify final state
    assert!(
        key_map.get(&b"key1".to_vec()).is_none(),
        "key1 should be deleted"
    );
    assert_eq!(
        key_map.get(&b"key2".to_vec()).unwrap().value(),
        b"value2_new",
        "key2 should have new value"
    );
    assert_eq!(
        key_map.get(&b"key3".to_vec()).unwrap().value(),
        b"value3",
        "key3 should exist"
    );
...
fn test_wal_corrupted_file() {
    ...
    assert_eq!(
        key_map.get(&b"key1".to_vec()).unwrap().value(),
        b"value1",
