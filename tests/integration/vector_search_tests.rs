...
fn test_vector_similarity_functions() -> Result<()> {
    ...
    db.execute("CREATE TABLE embeddings (id INTEGER PRIMARY KEY, text TEXT(50), embedding VECTOR(3))")?;
    ...
    let result = db.query("SELECT id, text, DOT_PRODUCT(embedding, [1.0, 0.0, 0.0]) FROM embeddings WHERE id = 1")?;
    let rows = result.rows();
    assert_eq!(rows.len(), 1);
    assert_eq!(rows[0][2], SqlValue::Real(1.0)); // Dot product should be 1.0
    
    // Test L2_NORMALIZE function
    let result = db.query("SELECT id, text, L2_NORMALIZE(embedding) FROM embeddings WHERE id = 4")?;
    let rows = result.rows();
    assert_eq!(rows.len(), 1);
...
fn test_vector_search_operations() -> Result<()> {
    ...
    db.execute("INSERT INTO embeddings (id, text, embedding) VALUES (3, 'bird', [0.0, 0.0, 1.0])")?;
    db.execute("INSERT INTO embeddings (id, text, embedding) VALUES (4, 'fish', [0.5, 0.5, 0.0])")?;
    db.execute("INSERT INTO embeddings (id, text, embedding) VALUES (5, 'mammal', [0.7, 0.3, 0.0])")?;
    ...
    let ids: Vec<i64> = rows.iter().map(|row| {
...
fn test_vector_edge_cases() -> Result<()> {
    ...
    let result = db.query("SELECT COSINE_SIMILARITY(vec, [1.0, 0.0, 0.0]) FROM edge_cases WHERE id = 2")?;
    let rows = result.rows();
    assert_eq!(rows.len(), 1);
    ...
    let result = db.query("SELECT DOT_PRODUCT(vec, [1.0, 1.0, 1.0]) FROM edge_cases WHERE id = 5")?;
    let rows = result.rows();
    assert_eq!(rows.len(), 1);
