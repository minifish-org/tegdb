        ...
        Statement::Update(update) => {
            match &update.assignments[0].value {
                Expression::BinaryOp { left, operator, right } => {
                    ...
                    match &**right {
                        Expression::BinaryOp { left: mult_left, operator: mult_op, right: mult_right } => {
        ...
        Statement::Update(update) => {
            match &update.assignments[0].value {
                Expression::BinaryOp { left, operator, right } => {
                    ...
                    match &**left {
                        Expression::BinaryOp { left: add_left, operator: add_op, right: add_right } => {
...
fn test_parse_literal_values_still_work() {
    ...
    match statement {
        Statement::Update(update) => {
            ...
            assert_eq!(update.assignments[0].value, Expression::Value(SqlValue::Text("John".to_string())));
            assert_eq!(update.assignments[1].value, Expression::Value(SqlValue::Integer(25)));
            assert_eq!(update.assignments[2].value, Expression::Value(SqlValue::Real(95.5)));
        }
        _ => panic!("Expected UPDATE statement"),
    }
}
