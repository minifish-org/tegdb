...
fn test_make_marker_key() {
    ...
    let test_cases = vec![
        ...
        (
            u64::MAX,
            format!("{}18446744073709551615", TXN_MARKER_PREFIX),
        ),
    ];
...
fn test_make_marker_key_format() {
    ...
    assert!(
        marker.starts_with(TXN_MARKER_PREFIX),
    ...
    assert!(
        snapshot_str.is_ok(),
        "Failed to parse snapshot number from marker key '{}'",
        marker
    );
    assert_eq!(
        snapshot_str.unwrap(),
        snapshot,
        "Snapshot number mismatch in marker key '{}'",
        marker
    );
}
