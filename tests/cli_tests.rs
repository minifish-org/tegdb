...
fn get_cli_binary() -> String {
    ...
    let binary_path = format!("{}/tg", target_dir);
...
fn test_cli_timer_and_echo() {
    ...
    assert!(stderr.contains("Query executed in") || stderr.contains("µs") || stderr.contains("ms")); // Timing output
}
...
fn test_cli_csv_import() {
    ...
    // Create table
    let setup_input = "CREATE TABLE users (id INTEGER PRIMARY KEY, name TEXT(50), age INTEGER)\n.quit\n";
    run_cli_command(&[db_path.to_str().unwrap()], Some(setup_input));
