...
impl SqlExecutionMetrics {
    fn new(operation: &str, parse_time: Duration, plan_time: Duration, execute_time: Duration, records: usize) -> Self {
        Self {
            operation: operation.to_string(),
...
fn setup_test_table(db: &mut Database, table_name: &str, record_count: usize) -> Result<PerformanceMetrics> {
    ...
    Ok(PerformanceMetrics::new("Table Setup", duration, total_inserted))
}
...
fn test_basic_crud_performance() -> Result<()> {
    ...
    for i in 0..record_count {
        db.execute(&format!("INSERT INTO perf_test (id, name, value) VALUES ({}, 'user{}', {})", i, i, i))?;
    }
    metrics.push(PerformanceMetrics::new("Single INSERT", start.elapsed(), record_count));
    
    // Test select all performance
    let start = Instant::now();
    let result = db.query("SELECT * FROM perf_test")?;
    let rows = result.collect_rows()?;
    metrics.push(PerformanceMetrics::new("SELECT ALL", start.elapsed(), rows.len()));
    ...
    metrics.push(PerformanceMetrics::new("SELECT FILTERED", start.elapsed(), filtered_rows.len()));
...
fn test_streaming_query_performance() -> Result<()> {
    ...
    metrics.push(PerformanceMetrics::new("Streaming Query", start.elapsed(), streaming_count));
    ...
    metrics.push(PerformanceMetrics::new("Batch Query", start.elapsed(), batch_rows.len()));
    ...
    metrics.push(PerformanceMetrics::new("Streaming Filtered", start.elapsed(), filtered_count));
...
fn test_transaction_performance() -> Result<()> {
    ...
    let record_count = 1000;
    for i in 0..record_count {
        db.execute(&format!("INSERT INTO tx_test (id, name, value) VALUES ({}, 'user{}', {})", i, i, i))?;
    }
    metrics.push(PerformanceMetrics::new("Auto-commit INSERTs", start.elapsed(), record_count));
    ...
    for i in 0..record_count {
        tx.execute(&format!("INSERT INTO tx_test (id, name, value) VALUES ({}, 'user{}', {})", i, i, i))?;
    }
    tx.commit()?;
    metrics.push(PerformanceMetrics::new("Transaction Batch", start.elapsed(), record_count));
    
    // Test transaction with rollback
    let start = Instant::now();
    let mut tx = db.begin_transaction()?;
    for i in 0..100 {
        tx.execute(&format!("INSERT INTO tx_test (id, name, value) VALUES ({}, 'temp{}', {})", i + 10000, i, i))?;
    }
    tx.rollback()?;
    metrics.push(PerformanceMetrics::new("Transaction Rollback", start.elapsed(), 100));
    ...
    tx.commit()?;
    metrics.push(PerformanceMetrics::new("Transaction Streaming", start.elapsed(), tx_streaming_count));
...
fn test_schema_operations_performance() -> Result<()> {
    ...
    metrics.push(PerformanceMetrics::new("Multiple CREATE TABLE", start.elapsed(), table_count));
    
    // Test schema cache refresh
    let start = Instant::now();
    db.refresh_schema_cache()?;
    metrics.push(PerformanceMetrics::new("Schema Cache Refresh", start.elapsed(), 1));
    
    // Test schema introspection
    let start = Instant::now();
    let schemas = db.get_table_schemas();
    metrics.push(PerformanceMetrics::new("Schema Introspection", start.elapsed(), schemas.len()));
    ...
    metrics.push(PerformanceMetrics::new("Multiple DROP TABLE", start.elapsed(), table_count));
...
fn test_large_dataset_performance() -> Result<()> {
    ...
    metrics.push(PerformanceMetrics::new("Full Table Scan", start.elapsed(), all_rows.len()));
    ...
    metrics.push(PerformanceMetrics::new("Selective Query", start.elapsed(), selected_rows.len()));
    ...
    metrics.push(PerformanceMetrics::new("Streaming with LIMIT", start.elapsed(), limited_count));
    
    // Test aggregation-style operation (counting)
    let start = Instant::now();
    let result = db.query("SELECT * FROM large_test WHERE value % 10 = 0")?;
    let filtered_rows = result.collect_rows()?;
    metrics.push(PerformanceMetrics::new("Modulo Filter", start.elapsed(), filtered_rows.len()));
...
fn test_concurrent_schema_access_performance() -> Result<()> {
    ...
    // Insert some data
    for i in 0..1000 {
        db.execute(&format!("INSERT INTO concurrent_test (id, data) VALUES ({}, 'data_{}')", i, i))?;
    }
    ...
    metrics.push(PerformanceMetrics::new("Rapid Schema Access", start.elapsed(), iterations));
...
fn test_memory_usage_pattern() -> Result<()> {
    ...
    metrics.push(PerformanceMetrics::new("Small Streaming Batch", start.elapsed(), small_batch_count));
    ...
    metrics.push(PerformanceMetrics::new("Medium Streaming Batch", start.elapsed(), medium_batch_count));
    
    // Large batch collection
    let start = Instant::now();
    let result = db.query("SELECT * FROM memory_test")?;
    let large_batch = result.collect_rows()?;
    metrics.push(PerformanceMetrics::new("Large Batch Collection", start.elapsed(), large_batch.len()));
...
#[cfg(feature = "dev")]
fn measure_sql_execution(db: &mut Database, sql: &str, operation_name: &str) -> Result<SqlExecutionMetrics> {
    // Parse timing
    let parse_start = Instant::now();
    let (_, statement) = parse_sql(sql)
        .map_err(|e| tegdb::Error::Other(format!("SQL parse error: {:?}", e)))?;
    ...
    Ok(SqlExecutionMetrics::new(operation_name, parse_duration, plan_duration, execute_duration, actual_records))
}
...
fn measure_transaction_sql_execution(
    ...
    Ok(SqlExecutionMetrics::new(operation_name, parse_duration, adjusted_plan_duration, adjusted_execute_duration, actual_records))
}
...
fn test_detailed_sql_pipeline_performance() -> Result<()> {
    ...
    let insert_sqls = vec![
        ("INSERT INTO pipeline_test (id, name, value) VALUES (1, 'user1', 100)", "Single INSERT"),
        ("INSERT INTO pipeline_test (id, name, value) VALUES (2, 'user with longer name', 200)", "INSERT with long text"),
    ];
    ...
    for i in 3..=50 {
        db.execute(&format!("INSERT INTO pipeline_test (id, name, value) VALUES ({}, 'user{}', {})", i, i, i * 10))?;
    }
    ...
    let select_queries = vec![
        ...
        ("SELECT * FROM pipeline_test WHERE value > 250", "SELECT with WHERE"),
        ("SELECT * FROM pipeline_test WHERE value > 100 AND value < 300", "SELECT with AND"),
        ("SELECT * FROM pipeline_test WHERE name LIKE '%user%'", "SELECT with LIKE"),
    ...
    let mutation_queries = vec![
        ("UPDATE pipeline_test SET name = 'updated' WHERE value < 150", "UPDATE with WHERE"),
        ("DELETE FROM pipeline_test WHERE value > 400", "DELETE with WHERE"),
    ];
    ...
    let mut parse_times: Vec<Duration> = detailed_metrics.iter().map(|m| m.parse_duration).collect();
    let mut plan_times: Vec<Duration> = detailed_metrics.iter().map(|m| m.plan_duration).collect();
    let mut execute_times: Vec<Duration> = detailed_metrics.iter().map(|m| m.execute_duration).collect();
...
fn test_transaction_pipeline_performance() -> Result<()> {
    ...
    let tx_operations = vec![
        ("INSERT INTO tx_pipeline_test (id, name, value) VALUES (1, 'tx_user1', 100)", "TX INSERT"),
        ("INSERT INTO tx_pipeline_test (id, name, value) VALUES (2, 'tx_user2', 200)", "TX INSERT 2"),
        ("SELECT * FROM tx_pipeline_test", "TX SELECT"),
        ("UPDATE tx_pipeline_test SET value = 150 WHERE id = 1", "TX UPDATE"),
        ("SELECT * FROM tx_pipeline_test WHERE value > 100", "TX SELECT filtered"),
    ];
...
fn test_parser_complexity_performance() -> Result<()> {
    ...
    // Setup test table
    db.execute("CREATE TABLE parse_test (id INTEGER PRIMARY KEY, name TEXT, value INTEGER, data TEXT)")?;
    ...
    let complex_queries = vec![
        ...
        ("SELECT * FROM parse_test WHERE id = 1", "SELECT with simple WHERE"),
        ("SELECT * FROM parse_test WHERE id > 1 AND name = 'test'", "SELECT with AND condition"),
        ("SELECT * FROM parse_test WHERE id > 1 OR value < 100", "SELECT with OR condition"),
        ("SELECT * FROM parse_test WHERE (id > 1 AND name = 'test') OR value < 100", "SELECT with complex WHERE"),
        ("SELECT id, name FROM parse_test WHERE value LIKE '%test%' LIMIT 10", "SELECT with LIKE and LIMIT"),
        ("INSERT INTO parse_test (id, name, value, data) VALUES (1, 'test', 100, 'some data')", "Complex INSERT"),
        ("UPDATE parse_test SET name = 'updated', value = 200 WHERE id = 1 AND value > 50", "Complex UPDATE"),
    ];
