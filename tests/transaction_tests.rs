...
async fn test_atomicity() -> Result<(), Error> {
    ...
    // Test successful atomic transaction
    {
        ...
        assert_eq!(txn2.select(b"key1").await.unwrap().0, Some(b"value1".to_vec()));
        assert_eq!(txn2.select(b"key2").await.unwrap().0, Some(b"value2".to_vec()));
        txn2.rollback().await?;
    }
...
async fn test_consistency() -> Result<(), Error> {
    ...
    // Test maintaining referential integrity
    {
        ...
        let mut txn2 = db.new_transaction().await;
        assert_eq!(txn2.select(b"parent").await.unwrap().0, Some(b"parent_value".to_vec()));
        assert_eq!(txn2.select(b"child").await.unwrap().0, Some(b"child_value".to_vec()));
        txn2.rollback().await?;
    }
...
async fn test_isolation() -> Result<(), Error> {
    ...
    assert_eq!(txn2.select(b"shared_key").await.unwrap().0, Some(b"value1".to_vec()));
    // Update the value
...
async fn test_durability() -> Result<(), Error> {
    ...
    // First phase: Write data and commit
    {
        let db = Database::new(path.clone()).await;
        let mut txn = db.new_transaction().await;
        txn.insert(b"durable_key", b"durable_value".to_vec()).await?;
        txn.commit().await?;
        db.engine.flush()?;
        db.shutdown().await;
    }
