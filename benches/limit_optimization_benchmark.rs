...
fn limit_optimization_benchmark(c: &mut Criterion) {
    ...
    group.bench_function(BenchmarkId::new("TegDB", "limit_5"), |b| {
        b.iter(|| {
            let result = tegdb.query("SELECT id, name FROM items LIMIT 5").unwrap().into_query_result().unwrap();
            black_box(result);
        });
    });
    ...
    group.bench_function(BenchmarkId::new("SQLite", "limit_5"), |b| {
        b.iter(|| {
            let mut stmt = sqlite.prepare("SELECT id, name FROM items LIMIT 5").unwrap();
    ...
    group.bench_function(BenchmarkId::new("TegDB", "limit_50"), |b| {
        b.iter(|| {
            let result = tegdb.query("SELECT id, name FROM items LIMIT 50").unwrap().into_query_result().unwrap();
            black_box(result);
        });
    });
    ...
    group.bench_function(BenchmarkId::new("SQLite", "limit_50"), |b| {
        b.iter(|| {
            let mut stmt = sqlite.prepare("SELECT id, name FROM items LIMIT 50").unwrap();
            let mut rows = stmt.query([]).unwrap();
            let mut results = Vec::new();
            while let Some(row) = rows.next().unwrap() {
                results.push((
    ...
    group.bench_function(BenchmarkId::new("TegDB", "limit_pk_filter"), |b| {
        b.iter(|| {
            let result = tegdb.query("SELECT id, name FROM items WHERE id >= 100 LIMIT 10").unwrap().into_query_result().unwrap();
            black_box(result);
        });
    });
    ...
    group.bench_function(BenchmarkId::new("SQLite", "limit_pk_filter"), |b| {
        b.iter(|| {
            let mut stmt = sqlite.prepare("SELECT id, name FROM items WHERE id >= 100 LIMIT 10").unwrap();
            let mut rows = stmt.query([]).unwrap();
            let mut results = Vec::new();
            while let Some(row) = rows.next().unwrap() {
                results.push((
    ...
    group.bench_function(BenchmarkId::new("TegDB", "limit_category_filter"), |b| {
        b.iter(|| {
            let result = tegdb.query("SELECT id, name FROM items WHERE category = 'A' LIMIT 5").unwrap().into_query_result().unwrap();
            black_box(result);
        });
    });
    ...
    group.bench_function(BenchmarkId::new("SQLite", "limit_category_filter"), |b| {
        b.iter(|| {
            let mut stmt = sqlite.prepare("SELECT id, name FROM items WHERE category = 'A' LIMIT 5").unwrap();
            let mut rows = stmt.query([]).unwrap();
            let mut results = Vec::new();
            while let Some(row) = rows.next().unwrap() {
                results.push((
    ...
    group.bench_function(BenchmarkId::new("TegDB", "full_scan"), |b| {
        b.iter(|| {
            let result = tegdb.query("SELECT id, name FROM items").unwrap().into_query_result().unwrap();
            black_box(result);
        });
    });
    ...
    group.bench_function(BenchmarkId::new("TegDB", "limited_scan"), |b| {
        b.iter(|| {
            let result = tegdb.query("SELECT id, name FROM items LIMIT 10").unwrap().into_query_result().unwrap();
            black_box(result);
        });
    });
    ...
    group.bench_function(BenchmarkId::new("SQLite", "limited_scan"), |b| {
        b.iter(|| {
            let mut stmt = sqlite.prepare("SELECT id, name FROM items LIMIT 10").unwrap();
            let mut rows = stmt.query([]).unwrap();
            let mut results = Vec::new();
            while let Some(row) = rows.next().unwrap() {
                results.push((
