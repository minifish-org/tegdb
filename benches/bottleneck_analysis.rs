...
fn bottleneck_analysis(c: &mut Criterion) {
    ...
    let serialized_data = tegdb::native_row_format::NativeRowFormat::serialize(&test_row_data, &test_schema).unwrap();
    ...
    c.bench_function("storage format serialize", |b| {
        b.iter(|| {
            let serialized = storage_format.serialize_row(
                black_box(&test_row_data), 
                black_box(&test_schema)
            ).unwrap();
            black_box(serialized);
    ...
    c.bench_function("storage format deserialize", |b| {
        b.iter(|| {
            let deserialized = storage_format.deserialize_row(
                black_box(&serialized_data), 
                black_box(&test_schema)
            ).unwrap();
            black_box(deserialized);
    ...
    for i in 1..=100 {
        db.execute(&format!("INSERT INTO test (id, value) VALUES ({}, {})", i, i * 2)).unwrap();
    }
