...
fn main() -> tegdb::Result<()> {
    ...
    print_performance_metric("Selective Columns (name, score)", results.selective_scan_time);
    ...
    println!("Average bytes per row: {:.1}", results.db_size_estimate as f64 / test_data.len() as f64);
    println!("Compression ratio: {:.1}:1", estimate_uncompressed_size(&test_data) as f64 / results.db_size_estimate as f64);
...
fn test_native_format(test_data: &[(i64, String, String, f64, i64)]) -> tegdb::Result<BenchmarkResults> {
...
fn analyze_performance(results: &BenchmarkResults, row_count: usize) {
    let rows_per_second_insert = (row_count as f64) / (results.insert_time as f64 / 1_000_000_000.0);
    let rows_per_second_scan = (row_count as f64) / (results.full_scan_time as f64 / 1_000_000_000.0);
    
    println!("• Insert throughput: {:.0} rows/second", rows_per_second_insert);
    ...
    println!("• Selective column scan is {:.1}x faster than full scan", selective_efficiency);
