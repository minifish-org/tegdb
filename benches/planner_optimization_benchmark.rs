...
fn temp_db_path(prefix: &str) -> PathBuf {
    ...
    path.push(format!("tegdb_planner_bench_{}_{}", prefix, std::process::id()));
    path
}
...
fn planner_optimization_benchmark(c: &mut Criterion) {
    ...
    group.bench_function(BenchmarkId::new("SQLite", "single_pk_lookup"), |b| {
        b.iter(|| {
            ...
            let mut stmt = sqlite.prepare("SELECT * FROM products WHERE id = ?").unwrap();
    ...
    group.bench_function(BenchmarkId::new("SQLite", "pk_range_small"), |b| {
        b.iter(|| {
            ...
            let mut stmt = sqlite.prepare("SELECT id, name, price FROM products WHERE id >= ? AND id <= ?").unwrap();
    ...
    group.bench_function(BenchmarkId::new("TegDB", "pk_with_additional_filter"), |b| {
        b.iter(|| {
    ...
    group.bench_function(BenchmarkId::new("SQLite", "pk_with_additional_filter"), |b| {
        b.iter(|| {
            let id = black_box(7500);
            let min_price = black_box(50.0);
            let mut stmt = sqlite.prepare("SELECT name, price FROM products WHERE id = ? AND price > ?").unwrap();
    ...
    group.bench_function(BenchmarkId::new("SQLite", "count_with_pk_range"), |b| {
        b.iter(|| {
            let start = black_box(1000);
            let end = black_box(2000);
            let mut stmt = sqlite.prepare("SELECT COUNT(*) FROM products WHERE id >= ? AND id <= ?").unwrap();
            let mut rows = stmt.query([start, end]).unwrap();
    ...
    group.bench_function(BenchmarkId::new("SQLite", "multi_condition_with_pk"), |b| {
        b.iter(|| {
            ...
            let mut rows = stmt.query([id.to_string(), min_price.to_string(), min_stock.to_string()]).unwrap();
            let mut results = Vec::new();
            while let Some(row) = rows.next().unwrap() {
                results.push((
                    row.get::<_, String>(0).unwrap(),
...
criterion_group!(
    planner_benches,
    planner_optimization_benchmark
);
